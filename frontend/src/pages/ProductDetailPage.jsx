...
export default function ProductDetailPage() {
  ...
  const tabContent = {
    ...
    'Đánh giá': visibleReviews.length ? (
      <div className="review-list">{visibleReviews.map((review) => <article key={review.id}><strong>{review.user?.fullName || 'Khách hàng'}</strong><span>{'★'.repeat(review.rating)}</span><p>{review.content}</p></article>)}</div>
    ...
    <div className="page product-detail-page">
      ...
      <section className="container product-detail-grid">
        <div className="product-detail-media">
          ...
          <div className="product-detail-trust"><span><BadgeCheck /> Kích hoạt trên tài khoản bạn quản lý</span><span><RefreshCcw />Chính sách hỗ trợ rõ ràng</span><span><Zap />Hướng dẫn kích hoạt chi tiết</span></div>
        </div>
        ...
        <div className="product-detail-info">
          ...
          {hasReviews && (
            <div className="rating-row rating-row--large">
              <Star fill="currentColor" />
              <strong>{averageRating.toFixed(1)}</strong>
              <span>({visibleReviews.length} đánh giá)</span>
            </div>
          )}
